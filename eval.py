...
def main():
    ...
    scores = evaluate(TC, MSVD, score_data_loader, decoder)
    print(scores)
...
def beam_search(config, n_vocabs, decoder, input, hidden, encoder_outputs):
    input_list = [ input ]
    hidden_list = [ hidden ]
    ...
    for t in range(config.caption_max_len + 1):
        ...
        topk_probs, topk_flat_idxs = outputs.topk(config.beam_width)
...
def evaluate(config, corpus, data_loader, decoder):
    total_vids = []
    total_pd_captions = []
    ...
    for batch in data_loader:
        ...
        if config.search_method == "greedy":
            ...
            output_indices = beam_search(config, corpus.vocab.n_vocabs, decoder, input, hidden, encoder_outputs)
